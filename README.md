...
版本 0.4.0。旧版独立重写的 CodexMonitorCore/App 已从工作树移除，历史备份保留。这次不是给重写版补标签。

## 页面与功能

主标签为 **Codex / 性能 / Skills / Codex Radar / 重置预测 / 远程账号**。远程账号统一承载 Codex 官方账号、网关上游账号、NewAPI 和 Sub2API 用户余额；各来源仍独立启停和保存配置，没有删除能力。Radar 标签可见不等于自动启用网络数据源。
...
- HUD 恢复单层圆角边界；“恢复外观默认设定”统一恢复显示模式、刘海/浮窗几何、透明度、展开动画和运行指示灯动画。
- 右侧自定义布局中的每个控件可以单独绑定本机或不同远程账号；未绑定控件继续使用默认数据源。账号卡不再承担“显示到 HUD”的切换。
- OpenAI 服务组件默认收起，需要时展开；Will Codex Reset 仅保留“前往网站邮件订阅”入口。
- Codex 官方账号卡保留接口数据源与读取时间，不展示内部工作区请求/返回字段。
- 子代理没有独立 token_count 时明确显示“未读到用量”，不把未知费用写成 $0；重置次数弹窗固定使用原有深色外观。
- “恢复默认刷新”只恢复本机 Codex 的平衡刷新频率，不会触发刷新，也不会修改账号、外观、布局或远程数据源。

## 0.4.0：官方状态、独立预测与 CLI 续跑

- **性能**：保留本机 CPU/内存，增加 OpenAI 官方状态摘要、Codex 组件、事件与数据新鲜度。启用后读取固定公开状态接口，每五分钟刷新。
- **重置预测**：独立标签，分别展示 Codex Reset Observatory 的 12/24/48/72 小时预测及 Will Codex Reset 的 48 小时参考评分。不平均、不把社区事件当个人额度恢复。来源过期或请求失败保留上次结果并标记。两个来源分别启停，默认不联网。
- **邮件**：Will 卡片只提供“前往网站订阅”链接，系统浏览器打开网站。订阅、退订由网站管理，本工具不读取邮箱、不申请邮箱权限、不接收或处理邮件。
- **CLI 续跑**：展开一个本机对话 → 检查并配置 → 核对账号、工作区、目录和权限 → 明确开启。可以在耗尽前开启监测，或在已暂停后开启等待。仅在明确的官方额度耗尽失败、同一执行账号新额度恢复、原会话未改变时调用 `codex exec ... resume <精确ID> -`；提示词通过 stdin 传递。预测、邮件和状态页不参与触发。每次额度事件最多发起一次，结果未知不自动重试。

续跑使用原数据目录的 **文件型 ChatGPT CLI 登录**（需要 `auth.json` 的账号/工作区信息），不会借用远程账户、API Key 或其他应用身份。Monitor 不直接改写登录文件；由用户授权启动的官方 CLI 仍可能按自身机制更新登录材料和会话记录。纯 Keychain 登录、缺失权限/模型上下文、外部提供商或改写过的 openai 路由暂不自动续跑。令牌过期时由用户在原 CODEX_HOME 重新登录。

默认 read-only；只有原任务允许 workspace-write 且用户确认后才允许工作区写入。不放宽原审批，不添加网络权限或额外写目录，不使用 --yolo。CLI 正常消耗额度并可能执行命令/修改工作区，因此不再将用户启用的续跑描述为“纯只读”。发送时请勿同时在 Desktop 操作同一对话；跨 Desktop/CLI 的最后瞬间并发不存在原子锁，检查与应用私有锁只能降低竞争，不能保证跨客户端精确一次执行。

退出 Monitor 会取消本应用的等待及其启动的 CLI；已发生的操作不回滚。进入额度等待后，对话被人工暂停/完成/继续、账号或模型配置改变时停止等待，不自动切账号、兑换次数或发送额外请求。界面“CLI 已完成”不代表 Desktop 窗口已同步或任务质量已验收。

详见 [本次规格](docs/specs/2026-09-11-status-prediction-cli-resume.md)、[任务](docs/tasks/2026-09-11-status-prediction-cli-resume.md) 和 [验证记录](docs/VALIDATION-status-prediction-cli.md)。
...
网络功能保留并按设置启用，不再宣称完全离线。性能后台采样默认关闭，Skills 可单独关闭；不使用内嵌网页登录，不接入邮箱或新增会话遥测；用户主动添加 Codex 官方账号时使用系统浏览器授权。截图只使用合成数据，不代表真实账户、网络服务和实机交互已经验收。
