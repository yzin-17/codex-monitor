...
续跑使用原数据目录的 **文件型 ChatGPT CLI 登录**（需要 `auth.json` 的账号/工作区信息），不会借用远程账户、API Key 或其他应用身份，也不修改登录文件。纯 Keychain 登录、缺失权限/模型上下文、外部提供商或改写过的 openai 路由暂不自动续跑。令牌过期时由用户在原 CODEX_HOME 重新登录。
...
退出 Monitor 会取消本应用的等待及其启动的 CLI；已发生的操作不回滚。对话暂停/完成/账号或模型配置改变时停止等待，不自动切账号、兑换次数或发送额外请求。界面“CLI 已完成”不代表 Desktop 窗口已同步或任务质量已验收。
