...
- 前五个标签常驻；配置并启用远程数据源后，标签栏出现第 6 个「远程账号」。
...
非刘海机型：菜单栏浮窗展开时，收起态 HUD 仍保留在顶部，面板从原位向下展开，深色半透明背景不截断壁纸。

![非刘海机型的菜单栏浮窗：顶部保留收起态 HUD，面板向下展开](docs/screenshots/panel-expanded.png)
...
- 启用某个预测来源后，只要该来源数据新鲜、无错误，且最高概率**超过 70%**，HUD 右侧就会出现橙色「预测 NN%」胶囊（下图：7d 额度仅剩 1%，社区预测 72%）。这只是提醒，**不会触发续跑或任何执行**。

![重置预测页：两个独立来源分别展示](docs/screenshots/prediction-tab.png)

预测超过 70% 时 HUD 上的提示（橙色胶囊，可悬停查看说明）：

![HUD 上的社区重置预测提示：预测 72%](docs/screenshots/hud-forecast-70.png)
