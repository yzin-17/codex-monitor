...
final class CodexNotchSettings: ObservableObject {
    nonisolated static let cliproxyKeychainService = "dev.yzin.codexmonitor.cliproxy.management-key"
    nonisolated static let cliproxyKeychainAccount = "default"
    nonisolated static let newAPIKeychainService = "dev.yzin.codexmonitor.newapi.password"
    nonisolated static let subAPIKeychainService = "dev.yzin.codexmonitor.subapi.password"
    ...
    var preferenceStore: UserDefaults { defaults }
    ...
    nonisolated private static func loadSecrets(
        ...
        migratedSecretVault = applyInitialOrLegacySecret(
            ...
            legacyLocations: [
                ...
                ("dev.yzin.codexmonitor.newapi.management-key", cliproxyKeychainAccount)
            ],
            vault: &loadedVault,
            ...
            let sources = decoded.map { source in
                ...
                do {
                    let legacySecret = try KeychainStore.read(
                        service: "dev.yzin.codexmonitor.remote-account-source",
                        account: copy.id
                    )
    ...
    nonisolated private static func balanceAccountKeychainService(for source: BalanceMonitorSource) -> String {
        switch source {
        case .newAPI:
            "dev.yzin.codexmonitor.newapi.account-password"
        case .subAPI:
            "dev.yzin.codexmonitor.subapi.account-password"
        }
    }
