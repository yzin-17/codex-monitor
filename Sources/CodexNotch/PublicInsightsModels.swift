...
enum PublicInsightSource: String, CaseIterable, Codable, Identifiable, Sendable {
    ...
    static let openAIIncidentEndpoint = URL(string: "https://status.openai.com/proxy/status.openai.com")!
    static let openAIStatusEndpoint = URL(string: "https://status.openai.com/api/v2/status.json")!
}
...
struct PublicStatusComponent: Codable, Equatable, Identifiable, Sendable {
    ...
    var label: String {
        ...
        case "major_outage", "full_outage": "严重故障"
...
enum PublicInsightParser {
    ...
    static func text(_ value: Any?, limit: Int = 500) -> String? {
        ...
        let clean = value
            .filter { !$0.isASCII || ($0.asciiValue ?? 0) >= 32 }
    ...
    static func parse(_ data: Data, source: PublicInsightSource, fetchedAt: Date = Date()) throws -> PublicInsightSnapshot {
        ...
        switch source {
        case .openAIStatus:
            return try parseClassicOpenAIStatus(root: root, fetchedAt: fetchedAt)
        case .observatory:
            guard let model = root["viewModel"] as? [String: Any] else {
                throw PublicInsightError.invalidResponse
            }
            var values: [Int: Double] = [:]
            for hours in [12, 24, 48, 72] {
                if let value = number(model["probability\(hours)h"]), (0...1).contains(value) {
                    values[hours] = value * 100
                }
            }
            guard !values.isEmpty,
                  let checked = date(root["checkedAt"] ?? model["lastUpdated"]) else {
                throw PublicInsightError.invalidResponse
            }
            ...
            var snapshot = PublicInsightSnapshot(
                ...
                summary: text(model["displayReasoningSummary"]) ?? "社区概率估计，仅供参考。",
            ...
            if let activity = root["latestTiboActivity"] as? [String: Any] {
                ...
                if let raw = text(activity["sourceUrl"], limit: 300),
                   ...
                   url.scheme == "https",
                   url.host == "x.com",
                   url.path.hasPrefix("/thsottiaux/status/") {
                    snapshot.latestTiboURL = url
                }
            }
            return snapshot
        ...
        case .willReset:
            guard let forecast = root["forecast"] as? [String: Any],
                  ...
                  let checked = date(root["fetchedAt"]) else {
                throw PublicInsightError.invalidResponse
            }
            ...
            let breakdown = (forecast["breakdown"] as? [[String: Any]] ?? [])
                .prefix(5)
                .compactMap { text($0["label"], limit: 120) }
            var snapshot = PublicInsightSnapshot(
                source: source,
                fetchedAt: fetchedAt,
                updatedAt: checked,
                upstreamStale: partial
                    || checked < fetchedAt.addingTimeInterval(-1800)
                    || checked > fetchedAt.addingTimeInterval(60),
                summary: (forecast["calibrated"] as? Bool == false
                          ? "未校准的社区评分，不代表可靠发生率。 "
                          : "社区预测，仅供参考。 ")
                    + (breakdown.isEmpty ? "不代表此账号已恢复额度。" : breakdown.joined(separator: " · ")),
                probabilities: [48: score]
            )
            snapshot.lastResetAt = date(forecast["latestResetAt"])
            if forecast["resetAnnounced"] as? Bool == true {
                snapshot.announcement = "该站报告存在重置预告；请以个人官方额度为准。"
            }
            return snapshot
        }
    }

    /// 解析 OpenAI incident.io 的原生分组结构。返回扁平数组，但保留 groupID，供性能页按组原地展开。
    static func parseOpenAIIncident(
        _ data: Data,
        statusData: Data? = nil,
        fetchedAt: Date = Date()
    ) throws -> PublicInsightSnapshot {
        guard data.count <= 2 * 1024 * 1024,
              let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let summary = root["summary"] as? [String: Any],
              let structure = summary["structure"] as? [String: Any],
              let items = structure["items"] as? [[String: Any]],
              !items.isEmpty else {
            throw PublicInsightError.invalidResponse
        }

        var stateByID: [String: String] = [:]
        for item in summary["affected_components"] as? [[String: Any]] ?? [] {
            guard let id = text(item["component_id"], limit: 100) else { continue }
            stateByID[id] = text(item["status"], limit: 60) ?? "degraded_performance"
        }

        func leaf(_ raw: [String: Any], groupID: String?, position: Int) -> PublicStatusComponent? {
            guard raw["hidden"] as? Bool != true,
                  let id = text(raw["component_id"], limit: 100),
                  let name = text(raw["name"], limit: 150) else { return nil }
            return .init(
                id: id,
                name: name,
                state: stateByID[id] ?? "operational",
                groupID: groupID,
                isGroup: false,
                position: position
            )
        ...
        for item in items {
            if let group = item["group"] as? [String: Any], group["hidden"] as? Bool != true {
                guard let id = text(group["id"], limit: 100),
                      let name = text(group["name"], limit: 150) else { continue }
                let children = (group["components"] as? [[String: Any]] ?? [])
                    .enumerated()
                    .compactMap { index, value in leaf(value, groupID: id, position: index) }
                let groupState = children.max(by: {
                    severityRank($0.state) < severityRank($1.state)
                })?.state ?? "operational"
                ...
                topPosition += 1
            } else if let component = item["component"] as? [String: Any],
                      let value = leaf(component, groupID: nil, position: topPosition) {
                components.append(value)
                topPosition += 1
            }
        }
        guard !components.isEmpty else { throw PublicInsightError.invalidResponse }

        let leaves = components.filter { $0.isGroup != true }
        let worstState = leaves.max(by: { severityRank($0.state) < severityRank($1.state) })?.state
            ?? "operational"
        var snapshot = PublicInsightSnapshot(
            source: .openAIStatus,
            fetchedAt: fetchedAt,
            updatedAt: nil,
            summary: worstState == "operational" ? "All Systems Operational" : "OpenAI 服务存在受影响组件",
            components: components
        )
        snapshot.overallIndicator = indicator(for: worstState)

        if let statusData,
           statusData.count <= 2 * 1024 * 1024,
           let overlay = try? JSONSerialization.jsonObject(with: statusData) as? [String: Any] {
            if let status = overlay["status"] as? [String: Any] {
                snapshot.summary = text(status["description"]) ?? snapshot.summary
            }
            snapshot.updatedAt = date((overlay["page"] as? [String: Any])?["updated_at"])
        }
        return snapshot
    }

    private static func parseClassicOpenAIStatus(
        root: [String: Any],
        fetchedAt: Date
    ) throws -> PublicInsightSnapshot {
        guard let status = root["status"] as? [String: Any],
              let indicator = text(status["indicator"], limit: 40),
              let raw = root["components"] as? [[String: Any]] else {
            throw PublicInsightError.invalidResponse
        }
        let parts = raw.prefix(300).compactMap { part -> PublicStatusComponent? in
            guard let id = text(part["id"], limit: 100),
                  let name = text(part["name"], limit: 150),
        ...
        var snapshot = PublicInsightSnapshot(
            ...
            summary: text(status["description"]) ?? "官方总体状态",
            components: parts,
            incidents: incidents
        )
    ...
    private static func severityRank(_ state: String) -> Int {
        switch state {
        case "operational": 0
        case "under_maintenance": 1
        case "degraded_performance": 2
        case "partial_outage": 3
        case "major_outage", "full_outage": 4
        default: 1
        }
    }

    private static func indicator(for state: String) -> String {
        switch severityRank(state) {
        case 0: "none"
        case 1, 2: "minor"
        case 3: "major"
        default: "critical"
