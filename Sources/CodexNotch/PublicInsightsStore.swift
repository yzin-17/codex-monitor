...
final class PublicInsightsStore: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard, automatic: Bool = true, fetcher: Fetcher? = nil) {
        self.defaults = defaults; self.automatic = automatic
        self.fetcher = fetcher ?? { try await PublicInsightsClient().fetch($0) }
        enabled = Set((defaults.stringArray(forKey: "publicInsights.enabled.v1") ?? []).compactMap(PublicInsightSource.init(rawValue:)))
        if let data = defaults.data(forKey: "publicInsights.cache.v1"), data.count <= 512 * 1024,
           let cache = try? JSONDecoder().decode([PublicInsightSnapshot].self, from: data) {
            for item in cache { snapshots[item.source] = item }
        }
        // 只有显式启用过的公共来源才轮询。预览和测试不启动网络。
        if automatic { schedule() }
    }
    func setEnabled(_ source: PublicInsightSource, _ value: Bool) {
        if value { enabled.insert(source) } else {
            enabled.remove(source); generations[source] = UUID(); tasks.removeValue(forKey: source)?.cancel(); refreshing.remove(source)
        }
    ...
    func refresh(_ source: PublicInsightSource) {
        ...
        let generation = UUID(); generations[source] = generation; refreshing.insert(source)
        tasks[source] = Task { [weak self, fetcher] in
            let result: Result<PublicInsightSnapshot, Error>
            do { result = .success(try await fetcher(source)) } catch { result = .failure(error) }
            guard let self, !Task.isCancelled, self.enabled.contains(source), self.generations[source] == generation else { return }
            self.tasks[source] = nil; self.refreshing.remove(source)
            switch result {
            case .success(let value):
                guard value.source == source else { self.errors[source] = PublicInsightError.invalidResponse.localizedDescription; return }
                self.snapshots[source] = value; self.errors[source] = nil
            ...
            case .failure(let error):
                self.errors[source] = (error as? PublicInsightError)?.localizedDescription ?? "连接失败，保留上次数据；不会将失败显示为正常。"
            }
        }
    }
